...
const Input = () => {
    ...
    useEffect(() => {
        ...
        console.log("storedR", storedResponse)
        if (storedResponse) {
            setResData(JSON.parse(storedResponse));
        }
        ...
        <div className="h-screen w-screen bg-white overflow-hidden">
            <div className="flex flex-col md:flex-row h-full gap-x-2">
                ...
                <div className="md:w-4/12 p-6 relative overflow-y-auto max-h-screen">
                    ...
                    <div className="shadow-lg rounded-3xl p-4 space-y-4 bg-[#21212]">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <input
                                ...
                                className="w-1/4 p-2 border rounded-md bg-slate-200 text-black focus:outline-none"
                            />
                            <input
                                type="date"
                                ...
                                min={today}
                                className="w-1/4 p-3 border rounded-md bg-slate-200 focus:outline-none"
                            />
                            <input
                                type="text"
                        ...
                        <div className="flex flex-wrap gap-6">
                            {budgetOptions.map((budget) => (
                                <div
                                    key={budget.name}
                                    className={`p-2 w-28 flex flex-col items-center justify-center rounded-lg cursor-pointer transition-all duration-300 ${selectedBudget === budget.name
                        ...
                        <div className="flex flex-wrap gap-6 mt-4">
                            {groupOptions.map((group) => (
                                <div
                                    key={group.name}
                                    className={`p-2 w-28 flex flex-col items-center justify-center rounded-lg cursor-pointer transition-all duration-300 ${selectedGroupType === group.name
                                        ? "bg-black text-white shadow-lg scale-105"
                                        : "bg-gray-200 text-gray-800"
                                        }`}
                        ...
                        {/* Activities */}
                        <div className="flex flex-wrap gap-6 mt-4">
                            {["Beaches", "City Tours", "Hiking", "Shopping", "Food Tours"].map(
                                (activity) => (
                                    <button
                                        key={activity}
                                        type="button"
                        ...
                        <div className="flex justify-center mt-4">
                            <button
                                className="btn glass w-32 text-white bg-gray-700"
                    ...
                    {/* Optionally, Home Image on the Left Side */}
                    {/* <div className="mt-8">
            <img src={home2} alt="home2" className="w-full object-cover shadow-md" />
          </div> */}
                </div>
                {/* Right Side - Itinerary or Placeholder Image (Scrollable & Animated Loader) */}
                <div className="md:w-8/12 bg-gray-100 p-4 overflow-y-auto max-h-screen">
                    {loading ? (
                        <motion.div
                        ...
                        >
                            <FaSpinner size={48} className="text-blue-500" />
                        </motion.div>
                    ) : resData ? (
                        ...
                        <img
                            src={home3}
                            alt="home"
                            className="w-full h-full object-cover rounded-br-[105px] shadow-md"
                        />
                    )}
                </div>
            </div>
